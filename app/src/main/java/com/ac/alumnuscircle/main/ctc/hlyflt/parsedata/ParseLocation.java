...
public class ParseLocation {
    ...
    public List<String> parseLocationLeftData() {
        ...
        result.add(result.size() / 2, "所有国家");
        if (!result.get(result.size() / 2).equals("所有国家")) {
            int index = result.indexOf("所有国家");
            String temp = result.get(result.size() / 2);
            result.set(index, temp);
            result.set(result.size() / 2, "所有国家");
        }
        return result;
    }
    ...
    public List<String> parseLocationMiddleData(String left) {
        ...
        if (left.equals("所有国家")) {
            ...
            result.add(result.size() / 2, "所有省份");
            if (!result.get(result.size() / 2).equals("所有省份")) {
                int index = result.indexOf("所有省份");
                String temp = result.get(result.size() / 2);
                result.set(index, temp);
                result.set(result.size() / 2, "所有省份");
            }
            return result;
        }
    }
    ...
    public List<String> parseLocationRightData(String left, String middle) {
        List<String> result = new ArrayList<String>();
        if ((!left.equals("所有国家")) && (middle == null || middle.length() == 0)) {
            NodeList countryNodeList = root.getElementsByTagName(COUNTRY);
            for (int i = 0; i < countryNodeList.getLength(); i++) {
                Element countryNode = (Element) (countryNodeList.item(i));
                if (countryNode.getAttribute("Name").equals(left)) {
            ...
            result.add(result.size() / 2, "所有城市");
            if (!result.get(result.size() / 2).equals("所有城市")) {
                int index = result.indexOf("所有城市");
                String temp = result.get(result.size() / 2);
                result.set(index, temp);
                result.set(result.size() / 2, "所有城市");
            }
            return result;
        }else if(left.equals("所有国家") || middle.equals("所有省份")){
            result.add(result.size() / 2, "所有城市");
            return result;
        } else {
            NodeList countryNodeList = root.getElementsByTagName(COUNTRY);
