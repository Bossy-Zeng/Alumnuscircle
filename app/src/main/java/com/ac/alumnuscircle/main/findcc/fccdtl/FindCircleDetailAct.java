                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        for (FindCircleDetail.DataBean.ResponseBean.ResultsBean res : results) {
                            ...
                            id.add(res.getId());
                            followed.add(res.getHas_followed());
...
//        noticeIntent.putExtra("CircleDetailImgurl",imagesUrl.get(position));
        if (followed.get(position).equals("False")) {
...
//            Toast.makeText(FindCircleDetailAct.this, "创建人是："+creator.get(position).getCreator_name(), Toast.LENGTH_SHORT).show();
            fansDialog.show();
        } else {
            Intent noticeIntent = new Intent(ActivityName.notice_NoticeAct);
            noticeIntent.putExtra("Id", id.get(position));
            noticeIntent.putExtra("ImageUrl", imagesUrl.get(position));
            noticeIntent.putExtra("Title", titles.get(position));
            noticeIntent.putExtra("Memeber", members.get(position).toString());
